...
const CodeBlockPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCodeBlock = async () => {
      try {
        const response = await axios.get<CodeBlock>(`https://tom-live-code.up.railway.app/codeblocks/${id}`);
  ...
  useEffect(() => {
    ...
    // Corrected student count update listener for both mentor and students
        ...
        <Row>
          <Col md={3}>
            <div className="role-permissions">
              ...
              <p>Students in Room: {studentCount}</p> {/* This will update for both mentor and students */}
            </div>
          </Col>
          <Col md={9}>
